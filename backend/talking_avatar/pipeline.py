...
from .enhance import finalize, restore_faces
...
def run_pipeline(job_id: str):
    ...
    try:
        ...
        _update(job_id, JobStage.AVATAR, 0.25, "Animating your photo")
        avatar_rel = generate_avatar.remote(job_id, emotion=params.get("emotion"))

        # 3. Lip-sync — MuseTalk 1.5 refinement.
        _update(job_id, JobStage.LIPSYNC, 0.55, "Refining lip-sync")
        video_rel = refine_lipsync.remote(job_id)

        # 4. Enhancement — GFPGAN face restoration (HQ mode only).
        if params.get("mode") == OutputMode.HIGH_QUALITY.value:
            _update(job_id, JobStage.ENHANCE, 0.75, "Restoring facial detail")
            video_rel = restore_faces.remote(job_id, video_rel)

        # 5. Finalize — mux, aspect ratio, compression.
        _update(job_id, JobStage.FINALIZE, 0.9, "Encoding MP4")
        finalize.remote(job_id, video_rel, params.get("aspect_ratio", "9:16"))
